...
function AdminDashboard() {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      try {
        ...
        console.error("Dashboard API fetch offline, using mock backup", err);
      }
    };
    loadDashboardData();
  }, []);
  ...
  const liveOrders = useMemo(() => {
    if (dbOrders.length === 0) return mockOrders;
  ...
  const stats = useMemo(() => {
    ...
    if (dbOrders.length > 0) {
      s[1].value = `${dbOrders.length}`;
      // Calculate total sales from all orders
      const totalAmount = dbOrders.reduce((sum, o) => sum + o.totalAmount, 0);
      s[0].value = formatINR(totalAmount);
    }
    s[3].value = `${lowStockProducts.length} Items`;
    return s;
  }, [dbOrders, lowStockProducts]);
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-2xl border border-[#e8dfd8] bg-white p-6 shadow-soft lg:col-span-2">
          ...
          {/* Custom SVG Chart (visual, safe, highly elegant responsive visual curves) */}
          <div className="mt-6 flex h-60 w-full flex-col justify-between">
            <div className="relative flex-1">
              <svg className="h-full w-full" viewBox="0 0 600 200" preserveAspectRatio="none">
                ...
                />
                {/* Active Dots */}
                <circle cx="200" cy="140" r="5" fill="#3a1d13" stroke="#d4af37" strokeWidth="2" />
                <circle cx="400" cy="60" r="5" fill="#3a1d13" stroke="#d4af37" strokeWidth="2" />
                <circle cx="600" cy="40" r="5" fill="#3a1d13" stroke="#d4af37" strokeWidth="2" />
              </svg>
            </div>
        ...
        <div className="rounded-2xl border border-[#e8dfd8] bg-white p-6 shadow-soft">
          ...
          <div className="mt-5 space-y-4">
            {lowStockProducts.length === 0 ? (
              <p className="text-center text-xs text-muted-foreground py-8">All sarees are in abundant supply.</p>
            ) : (
